...
private:
	...
	{
		if (coloured_)
			SetConsoleTextAttribute(console_, CurrentStyle);
		else
	...
	void GetColours()
	{
		CurrentStyle = GetColour(attr0_);
		if (attr1_)
			CurrentStyle |= GetColour(attr1_);
		if (attr2_)
			CurrentStyle |= GetColour(attr2_);
		if (attr3_)
			CurrentStyle |= GetColour(attr3_);
	}
